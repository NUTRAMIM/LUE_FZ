-- View de custo de IA por loja. Consulta: SELECT * FROM vw_custo_por_loja;
-- Preços USD jun/2026 por modelo; custo_brl usa câmbio fixo de 5.5 (ajuste se
-- precisar). Linhas antigas (model='desconhecido', cached=0) caem no preço
-- default (mini, sem cache); dados gravados após o deploy novo ficam exatos.
CREATE OR REPLACE VIEW vw_custo_por_loja AS
...
SELECT s.store_name,
       ps.calls,
       COALESCE(g.ia_mensagens, 0)  AS ia_mensagens,
       COALESCE(g.atendimentos, 0)  AS atendimentos,
       CASE WHEN COALESCE(g.atendimentos, 0) > 0
            THEN ROUND((ps.usd / g.atendimentos)::numeric, 4) END AS usd_por_atendimento,
       ps.prompt_tokens, ps.cached_tokens, ps.completion_tokens,
